...
- `live/` — live-only helpers used by `<LiveReload>`: `live-events.js` (SSE/reload primitives), `live-scroll.js` (follow-scroll helpers), `live-stats.js` (header stats), and `chat-preview.js` (streaming-preview helper, also used by `<BtwPopup>`)
