(function () {
  ...
  function formatToolCall(name, args) {
    ...
    switch (name) {
      ...
      case "write":
        return (
          "[write: " +
          shortenPath(String(args.path || args.file_path || "")) +
          "]"
        );
      case "edit":
        return (
  ...
  function renderToolOutput(text) {
    ...
    return (
      '<div class="tool-output expandable" onclick="if(window.getSelection().toString())return;this.classList.toggle(\'expanded\')"><div class="output-preview"><pre>' +
      escapeHtml(preview) +
      '</pre></div><div class="output-full"><pre>' +
      escapeHtml(text) +
      "</pre></div></div>"
    );
  }
  ...
  function renderToolCall(call, result) {
    ...
    function getResultImages() {
      if (!result) return [];
      return (result.content || []).filter(function (c) {
        return c.type === "image";
      });
    }
    ...
    if (call.name === "bash") {
      var cmd =
        typeof args.command === "string" ? escapeHtml(args.command) : invalid;
      ...
      var fp =
        typeof (args.file_path || args.path) === "string"
          ? shortenPath(String(args.file_path || args.path || ""))
          : invalid;
      html +=
        '<div class="tool-header"><span class="tool-name">read</span> <span class="tool-path">' +
        escapeHtml(fp) +
        "</span></div>";
      if (result) {
        ...
        imgs.forEach(function (img) {
          html +=
            '<img src="data:' +
            escapeHtml(img.mimeType || "image/png") +
            ";base64," +
            img.data +
            '" class="tool-image" />';
        });
      ...
      var fp =
        typeof (args.file_path || args.path) === "string"
          ? escapeHtml(shortenPath(String(args.file_path || args.path || "")))
          : invalid;
      html +=
        '<div class="tool-header"><span class="tool-name">write</span> <span class="tool-path">' +
        fp +
        "</span></div>";
      ...
      var dp =
        typeof args.path === "string"
          ? escapeHtml(shortenPath(String(args.path || ".")))
          : invalid;
      html +=
        '<div class="tool-header"><span class="tool-name">ls</span> <span class="tool-path">' +
        dp +
        "</span></div>";
      if (result) {
        var out = getResultText().trim();
      ...
      var qaAnswers =
        result && result.details && result.details.answers
          ? result.details.answers
          : {};
      var qaCancelled =
        result && result.details && result.details.cancelled === true;
      ...
      html +=
        '<div class="ask-question-card" data-question-count="' +
        questions.length +
        '">';
      ...
      questions.forEach(function (q, qi) {
        ...
        html +=
          '<div class="ask-question-block" data-question-text="' +
          ...
          options.forEach(function (opt) {
            var label =
              opt && typeof opt.label === "string"
                ...
                : String(opt || "");
            var desc =
              opt && typeof opt.description === "string" ? opt.description : "";
            var sel =
              answer === label ||
              (typeof answer === "string" &&
                answer.split(", ").indexOf(label) >= 0);
            var tag = qaInteractive ? "button" : "div";
            var cls =
              "ask-question-option" +
              (sel ? " selected" : "") +
              (qaInteractive ? " ask-question-option-action" : "");
            var dAttrs = qaInteractive
              ? ' type="button" data-question="' +
                escapeHtml(questionText) +
                '" data-answer="' +
                escapeHtml(label) +
                '"'
            ...
            if (desc)
              html +=
                '<div class="ask-question-option-desc">' +
                escapeHtml(desc) +
                "</div>";
            html += "</" + tag + ">";
          });
        ...
        if (answer)
          html +=
            '<div class="ask-question-answer"><span>Answer:</span> ' +
      ...
      if (qaInteractive) {
        if (qaMulti) {
          ...
          html +=
            '<div class="ask-question-hint">Use the chat composer below to answer this question.</div>';
        }
      }
      html += "</div>";
    } else {
      html +=
        '<div class="tool-header"><span class="tool-name">' +
  ...
  function renderEntry(entry, allEntries) {
    ...
    if (entry.type === "message") {
      ...
      if (msg.role === "user") {
        ...
        if (Array.isArray(content)) {
          var imgs = content.filter(function (c) {
            return c.type === "image";
          });
          if (imgs.length) {
        ...
        var text =
          typeof content === "string" ? content : extractContent(content);
        if (text.trim())
          html +=
            '<div class="markdown-content">' + safeMarked(text) + "</div>";
        html += "</div>";
        return html;
      }
      if (msg.role === "assistant") {
        ...
        if (msg.stopReason === "aborted")
          ...
          html +=
            ...
            escapeHtml(msg.errorMessage || "Unknown error") +
            "</div>";
        html += "</div>";
        return html;
      }
      if (msg.role === "bashExecution") {
        ...
        if (msg.cancelled)
          ...
          html +=
            '<div style="color:var(--error)">(exit ' + msg.exitCode + ")</div>";
        html += "</div>";
        return html;
      }
    ...
    if (entry.type === "model_change") {
      return (
        '<div class="model-change" id="' +
        eid +
        '">' +
        tsHtml +
        'Switched to model: <span class="model-name">' +
    ...
    if (entry.type === "compaction") {
      return (
        '<div class="compaction" id="' +
        eid +
        '"><div class="compaction-label">[compaction]</div><div class="compaction-collapsed">Compacted from ' +
    ...
    if (entry.type === "branch_summary") {
      return (
        '<div class="branch-summary" id="' +
        eid +
        '">' +
        tsHtml +
        '<div class="branch-summary-header">Branch Summary</div><div class="markdown-content">' +
        safeMarked(entry.summary || "") +
        "</div></div>"
      );
    }
    if (entry.type === "custom_message" && entry.display) {
      return (
        '<div class="hook-message" id="' +
        eid +
        '">' +
        tsHtml +
        '<div class="hook-type">[' +
        escapeHtml(entry.customType) +
        ']</div><div class="markdown-content">' +
        safeMarked(
  ...
  function isAtBottom() {
    ...
    var scrolled =
      window.scrollY || window.pageYOffset || de.scrollTop || body.scrollTop;
  ...
  function scrollElementAboveComposer(el, smooth) {
    ...
    if (viewportDelta > 0) {
      window.scrollTo({
        top: (window.scrollY || window.pageYOffset) + viewportDelta,
        behavior: smooth ? "smooth" : "auto",
      });
    }
  }
  ...
  function updateStats(entries) {
    ...
    entries.forEach(function (e) {
      ...
      if (m.role === "assistant") {
        assistant++;
        if (m.model)
          models.add(m.provider ? m.provider + "/" + m.model : m.model);
        if (m.usage) {
        ...
        toolCalls += (m.content || []).filter(function (c) {
    ...
    rows.forEach(function (row) {
      ...
      if (text.indexOf("Messages:") >= 0)
        val.textContent = parts.join(", ") || "0";
      if (text.indexOf("Tool Calls:") >= 0) val.textContent = toolCalls;
      if (text.indexOf("Models:") >= 0)
        val.textContent = Array.from(models).join(", ") || "unknown";
      ...
      if (text.indexOf("Cost:") >= 0)
        val.textContent = "$" + totalCost.toFixed(3);
    });
  }
  ...
  function renderChatPreview(payload) {
    ...
    if (!chatPreviewEl) {
      ...
      chatPreviewEl.innerHTML =
        '<div class="message-content assistant-text"></div><div class="preview-label">working<span class="working-dots" aria-hidden="true"></span></div>';
  ...
  function showShareResult(gistUrl, previewUrl) {
    ...
    function copyShareUrl(text, label) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard
          .writeText(text)
          .then(function () {
            showShareCopiedNotice(label, text);
          })
          .catch(fallbackCopy);
      } else {
        fallbackCopy();
    ...
    document
      .getElementById("share-copy-gist")
      .addEventListener("click", function () {
        copyShareUrl(gistUrl, "Gist");
      });
    document
      .getElementById("share-copy-preview")
      .addEventListener("click", function () {
  ...
  function showShareError(msg) {
    ...
    document.body.appendChild(shareOverlay);
    document
      .getElementById("share-close-err")
      .addEventListener("click", function () {
        shareOverlay.remove();
        shareOverlay = null;
      });
