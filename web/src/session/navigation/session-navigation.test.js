...
describe('session navigator', () => {
  it('renders header and path messages and wires copy buttons', () => {
    ...
    const nav = createSessionNavigator({
      ...
      renderEntry: (entry) => `<div id="entry-${entry.id}"><button class="copy-link-btn" data-entry-id="${entry.id}">copy</button></div>`,
    ...
    document.querySelector('[data-entry-id="a"]').click();
