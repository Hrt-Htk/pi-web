...
export function getTreeNodeDisplayHtml(entry, label, { extractContent, toolCallMap = new Map(), escapeHtmlImpl = escapeHtml } = {}) {
  ...
  switch (entry.type) {
    case 'message': {
      ...
      if (msg.role === 'assistant') {
        ...
        if (textContent) return labelHtml + `<span class="tree-role-assistant">assistant:</span> ${escapeHtmlImpl(textContent)}`;
        ...
        if (msg.errorMessage) return labelHtml + `<span class="tree-role-assistant">assistant:</span> <span class="tree-error">${escapeHtmlImpl(truncate(msg.errorMessage))}</span>`;
      ...
      if (msg.role === 'toolResult') {
        ...
        if (toolCall) return labelHtml + `<span class="tree-role-tool">${escapeHtmlImpl(formatToolCall(toolCall.name, toolCall.arguments))}</span>`;
    ...
    case 'compaction': return labelHtml + `<span class="tree-compaction">[compaction: ${Math.round(entry.tokensBefore / 1000)}k tokens]</span>`;
    case 'branch_summary': {
      const summary = truncate(normalize(entry.summary || ''));
      return labelHtml + `<span class="tree-branch-summary">[branch summary]:</span> ${escapeHtmlImpl(summary)}`;
    }
    case 'custom_message': {
      const content = typeof entry.content === 'string' ? entry.content : getContent(entry.content);
      return labelHtml + `<span class="tree-custom">[${escapeHtmlImpl(entry.customType)}]:</span> ${escapeHtmlImpl(truncate(normalize(content)))}`;
    }
    case 'model_change': return labelHtml + `<span class="tree-muted">[model: ${entry.modelId}]</span>`;
    case 'thinking_level_change': return labelHtml + `<span class="tree-muted">[thinking: ${entry.thinkingLevel}]</span>`;
    default: return labelHtml + `<span class="tree-muted">[${entry.type}]</span>`;
  }
}
