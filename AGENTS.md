...
- **Restarting the server:** always kill the old PID and start the new binary in a **single command** — never ask the user to start it manually. Use: `taskkill //PID <pid> //F && start "" pi-web.exe` (find PID via `tasklist | grep pi-web`).
